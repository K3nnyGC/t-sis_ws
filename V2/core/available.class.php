...
Class Available {
   function __construct($code_available_time,$dni,$date,$hour,$status_available) {
...
Class AvailableManager extends Conection {
   ...
   public function create ($code_available_time,$dni,$date,$hour,$status_available) {
      
         $data = $this->make_query("INSERT INTO $this->table (code_available_time, dni, `date`, `hour`, status_available ) VALUES ('', '$dni', '$date', $hour, $status_available )");
         ...
         if($data){
            ...
            return new Available($code_available_time,$dni,$date,$hour,$status_available );
         } else {
            return false;
         }
   ...
   public function findByPK($dni,$date,$hour){
      $data = $this->make_query("SELECT * FROM $this->table WHERE dni='$dni' AND `date` = '$date' AND `hour` = $hour ");
      if ($data){
         if ($row = $data->fetch_assoc()){
            return new Available($row['code_available_time'],
                               $row['dni'],
   ...
   public function update($available){
         ...
         $data = $this->make_query("UPDATE $this->table SET dni = $dni, `date` = '$date', `hour` = $hour, status_available = $status_available WHERE code_available_time=$code_available_time ");
