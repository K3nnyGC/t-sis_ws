...
Class Student {
   function __construct($dni,$name,$lastname,$email,$address,$password,$phone,$token,$picture,$status_student) {
      $this->dni = $dni;
      $this->name = $name;
...
Class StudentManager extends Conection {
   ...
   public function create ($dni,$name,$lastname,$email,$address,$password,$phone,$token,$picture,$status_student) {
      
         $data = $this->make_query("INSERT INTO $this->table (dni, name, lastname, email, address, password, phone, token, picture, status_student ) VALUES ('$dni', '$name', '$lastname', '$email', '$address', md5('$password'), '$phone', '$token', '$picture', $status_student )");

         if($data){
            return new Student($dni,$name,$lastname,$email,$address,md5($password),$phone, $token, $picture, $status_student);
         } else {
            return false;
         }
   ...
   public function update($student){
         ...
         $data = $this->make_query("UPDATE $this->table SET name = '$name', lastname = '$lastname', email = '$email', address = '$address', password = '$password', phone = '$phone', token = '$token', picture = '$picture', status_student = $status_student WHERE dni='$dni' ");
      ...
      if (isset($dni)&isset($tag)){
        switch ($tag) {
            case "contracts":
              ...
              $student = $this->uc->findById($dni);
              if ($student){
                $contracts = $cm->showByStudent($student->dni);
                if($contracts){
                  $contractsarray=[];
      ...
      if ($this->keys[count($this->keys)-1]=="student"){
         ...
         if ($obligatorios){
            ...
            if (!$this->uc->findById($vpost['dni'])){
               $student = $this->uc->create($vpost['dni'],
                          $vpost['name'],
                          $vpost['lastname'],
                          $vpost['email'],
                          $vpost['address'],
      ...
      if ($this->keys[count($this->keys)-2]=="student"){
        ...
        //isset($vpost['token'])||
         ...
         if ($opciones) {
            ...
            $student = $this->uc->findById($vpost['dni']);
            ...
            if ($student) {
               ...
               if (isset($vpost['password'])){
                if (md5($vpost['password'])==$student->password){
                  $vpost['password']=$student->password;
                } else {
               ...
               $studentA = $this->uc->update(new Student($vpost['dni'],
                                                         $vpost['name'],
                                                         $vpost['lastname'],
                                                         $vpost['email'],
                                                         $vpost['address'],
